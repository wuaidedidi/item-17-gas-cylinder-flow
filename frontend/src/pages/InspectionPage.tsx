import { PlusOutlined, ReloadOutlined, SearchOutlined } from '@ant-design/icons';
import { Button, DatePicker, Form, Input, Modal, Select, Space, Switch, Table } from 'antd';
import type { ColumnsType } from 'antd/es/table';
import dayjs from 'dayjs';
import { useEffect, useMemo, useState } from 'react';
import { http, showSuccess } from '../api/http';
import { recordStatusLabels } from '../api/labels';
import { recordStatusOptions } from '../api/options';
import { EntityTag } from '../components/EntityTag';
import { PageHeader } from '../components/PageHeader';
import { cleanPayload } from '../utils/form';

export const InspectionPage = () => {
  const [queryForm] = Form.useForm();
  const [form] = Form.useForm();
  const [items, setItems] = useState<any[]>([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [loading, setLoading] = useState(false);
  const [modalOpen, setModalOpen] = useState(false);
  const [cylinders, setCylinders] = useState<any[]>([]);

  const loadCylinders = async () => {
    const data = await http.get<any, any>('/cylinders', { params: { page: 1, pageSize: 100 } });
    setCylinders(data.list);
  };

  const load = async (nextPage = page, nextPageSize = pageSize) => {
    setLoading(true);
    try {
      const data = await http.get<any, any>('/records/inspection', {
        params: { ...queryForm.getFieldsValue(), page: nextPage, pageSize: nextPageSize },
      });
      setItems(data.list);
      setTotal(data.total);
      setPage(data.page);
      setPageSize(data.pageSize);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadCylinders();
    load(1, pageSize);
  }, []);

  const submit = async () => {
    const values = await form.validateFields();
    await http.post('/records/inspection', cleanPayload(values));
    showSuccess(values.isAbnormal ? '巡检记录已创建，并已生成预警' : '巡检记录已创建');
    setModalOpen(false);
    form.resetFields();
    await load(1, pageSize);
  };

  const columns: ColumnsType<any> = useMemo(
    () => [
      { title: '记录编号', dataIndex: 'recordNo', width: 180, fixed: 'left' },
      { title: '气瓶编码', dataIndex: ['cylinder', 'code'], width: 180 },
      { title: '气瓶名称', dataIndex: ['cylinder', 'name'], width: 140 },
      { title: '巡检时间', dataIndex: 'inspectedAt', width: 170, render: (v) => dayjs(v).format('YYYY-MM-DD HH:mm') },
      { title: '巡检员', dataIndex: 'inspectorName', width: 120 },
      { title: '异常', dataIndex: 'isAbnormal', width: 90, render: (v) => (v ? <EntityTag value="HIGH" labels={{ HIGH: '异常' }} /> : <EntityTag value="COMPLETED" labels={{ COMPLETED: '正常' }} />) },
      { title: '状态', dataIndex: 'status', width: 120, render: (v) => <EntityTag value={v} labels={recordStatusLabels} /> },
      { title: '结果', dataIndex: 'result', ellipsis: true },
      { title: '异常描述', dataIndex: 'issues', ellipsis: true },
    ],
    [],
  );

  return (
    <div className="page-stack">
      <PageHeader
        title="安全巡检管理"
        description="记录现场巡检结果，异常巡检会自动生成风险预警并进入安全员处理流程。"
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setModalOpen(true)}>新增巡检</Button>}
      />
      <section className="content-panel">
        <Form form={queryForm} className="toolbar-form" layout="inline" onFinish={() => load(1, pageSize)}>
          <Form.Item name="keyword">
            <Input prefix={<SearchOutlined />} placeholder="记录编号、气瓶编码" allowClear />
          </Form.Item>
          <Form.Item name="status">
            <Select placeholder="全部状态" allowClear options={recordStatusOptions} className="filter-select" />
          </Form.Item>
          <Space>
            <Button type="primary" htmlType="submit" icon={<SearchOutlined />}>查询</Button>
            <Button icon={<ReloadOutlined />} onClick={() => { queryForm.resetFields(); load(1, pageSize); }}>重置</Button>
          </Space>
        </Form>
        <Table
          rowKey="id"
          loading={loading}
          columns={columns}
          dataSource={items}
          scroll={{ x: 1180 }}
          pagination={{
            current: page,
            pageSize,
            total,
            showSizeChanger: true,
            showTotal: (count) => `共 ${count} 条`,
            onChange: load,
          }}
        />
      </section>

      <Modal title="新增巡检记录" open={modalOpen} width={760} onCancel={() => setModalOpen(false)} onOk={submit} okText="保存" cancelText="取消">
        <Form form={form} layout="vertical" requiredMark className="modal-grid-form" initialValues={{ isAbnormal: false, status: 'COMPLETED' }}>
          <Form.Item name="cylinderId" label="气瓶" rules={[{ required: true, message: '请选择气瓶' }]}>
            <Select showSearch placeholder="请选择气瓶" optionFilterProp="label" options={cylinders.map((item) => ({ value: item.id, label: `${item.code} · ${item.name}` }))} />
          </Form.Item>
          <Form.Item name="inspectedAt" label="巡检时间" rules={[{ required: true, message: '请选择巡检时间' }]}>
            <DatePicker showTime className="full-control" />
          </Form.Item>
          <Form.Item name="inspectorName" label="巡检员" rules={[{ required: true, message: '请输入巡检员' }]}>
            <Input placeholder="请输入巡检员" />
          </Form.Item>
          <Form.Item name="status" label="状态">
            <Select options={recordStatusOptions} />
          </Form.Item>
          <Form.Item name="isAbnormal" label="是否异常" valuePropName="checked">
            <Switch checkedChildren="异常" unCheckedChildren="正常" />
          </Form.Item>
          <Form.Item name="nextInspectionAt" label="下次巡检">
            <DatePicker className="full-control" />
          </Form.Item>
          <Form.Item name="result" label="巡检结果" className="form-span-2" rules={[{ required: true, message: '请输入巡检结果' }]}>
            <Input.TextArea rows={3} placeholder="请输入巡检结果" />
          </Form.Item>
          <Form.Item name="issues" label="异常描述" className="form-span-2">
            <Input.TextArea rows={3} placeholder="如存在异常，请填写异常描述" />
          </Form.Item>
          <Form.Item name="remark" label="备注" className="form-span-2">
            <Input.TextArea rows={2} placeholder="请输入备注" />
          </Form.Item>
        </Form>
      </Modal>
    </div>
  );
};

