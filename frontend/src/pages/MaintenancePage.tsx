...
export const MaintenancePage = () => {
  ...
  const load = async (nextPage = page, nextPageSize = pageSize) => {
    setLoading(true);
    try {
      const data = await http.get<any, any>('/records/maintenance', {
  ...
  const submit = async () => {
    ...
    await http.post('/records/maintenance', cleanPayload(values));
    showSuccess('维修保养记录已创建');
    setModalOpen(false);
    form.resetFields();
  ...
  const columns: ColumnsType<any> = useMemo(
    () => [
      ...
      { title: '保养时间', dataIndex: 'maintainedAt', width: 170, render: (v) => dayjs(v).format('YYYY-MM-DD HH:mm') },
      { title: '维修人员', dataIndex: 'technicianName', width: 120 },
      { title: '状态', dataIndex: 'status', width: 120, render: (v) => <EntityTag value={v} labels={recordStatusLabels} /> },
      { title: '维修结果', dataIndex: 'result', width: 140, ellipsis: true },
      { title: '维修内容', dataIndex: 'content', ellipsis: true },
    ],
    [],
  );
  ...
  return (
    <div className="page-stack">
      <PageHeader
        title="维修保养管理"
        description="形成维修保养台账，保留处理过程并同步气瓶当前维修状态。"
        extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => setModalOpen(true)}>新增保养</Button>}
      />
      ...
      <Modal title="新增维修保养记录" open={modalOpen} width={760} onCancel={() => setModalOpen(false)} onOk={submit} okText="保存" cancelText="取消">
        <Form form={form} layout="vertical" requiredMark className="modal-grid-form" initialValues={{ status: 'COMPLETED' }}>
          ...
          <Form.Item name="maintainedAt" label="保养时间" rules={[{ required: true, message: '请选择保养时间' }]}>
            <DatePicker showTime className="full-control" />
          </Form.Item>
          <Form.Item name="technicianName" label="维修人员" rules={[{ required: true, message: '请输入维修人员' }]}>
          ...
          <Form.Item name="content" label="维修内容" className="form-span-2" rules={[{ required: true, message: '请输入维修内容' }]}>
